...
AWT 에서 지원하는 배치는 다음과 같이 5가지 유형이 있다.1
1) FlowLayout : 행방향으로 늘어서는 배치 방식, 만일 공간이 부족하면 다음 행으로
				내려간다. 컨테이너의 크기에 따라 흐르듯 배치가 되므로 flow
				라는 뜻이 부여되었다..
2) BorderLayout : 북,동,서,남,중앙 이라는 바우이를 갖는 배치방법
3) GridLayout : 격자 즉 바둑판 모양의 배치방법 
4) CardLayout : 카드처럼 하나의 카드가 보일때는 다른 카드가 안보이는 방식의
				배치
5) GridBagLayout : GridLayout을 보다 상세하게 제어할 수 있는 배치방식
사실상 1,2,3번만으로도 충분히 개발가능함..
*/
package gui;
...
class BorderTest{
	public static void main(String[] args){
		...
		frame.add(bt_center, BorderLayout.CENTER); //가운데 버튼 부착
