...
public class Testing {
   ...
   public void createFolder(JPanel panel, String folderName, int fontSize) {
			...
			p_chat_south_center.setPreferredSize(new Dimension(240, p_chat_south_center.getHeight()+40));
			p_chat_south_center.updateUI();
...
//      p_west_south_chat.setLayout();
       ...
       *  서치 팝업창 panel_pop_container
       ------------------------------------------------------------------------------------*/
  	panel_pop_container = new JPanel();
	...
	bt_chat_pop_ok.addActionListener((e)->{
      ...
      popup_search = popupFactory.getPopup(frame, panel_pop_container,1065,211);
      btnSearch.addActionListener((e)->{
    	  int x = btnSearch.getLocationOnScreen().x;
    	  int y = btnSearch.getLocationOnScreen().y;
    	  if(s_pop == false) {
    		  popup_search = popupFactory.getPopup(frame, panel_pop_container, x-240,y);
    		  popup_search.show();
    		  Testing.s_pop = true;
    	  }else{
    		  Testing.s_pop = false;
