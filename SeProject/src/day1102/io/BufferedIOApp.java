 ...
 * 마치 cmd 창의 버퍼처럼 , 메모리상의 버퍼에 데이터를 모아놓고, 한줄이 끝나는 시점에만 
 * 입력 및 출력을 처리하면 프로그램 실행의 효율성이 극대화 됨 !!
 * 스트림 중 버퍼를 지원하는 스트림은 접두어로 Buffered~~ 가 붙는다
 * 
 * 실습 ) 키보드로 입련된 데이터를 한줄씩 가져와 출력해 보자~
 * */
...
public class BufferedIOApp {
	...
	public static void main(String[] args) {
		//keyboard 에 연결 된 스트림은 개발자가 원하는 타임에 new 할 수 없다.
		...
		BufferedReader buffer = new BufferedReader(reader); //버퍼처리된 문지기반의 입력스트림
		int data;
		String str = null;
		try {
			while(true) {
				str = buffer.readLine(); // 1문자 읽음 
